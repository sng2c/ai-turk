...
`turkctl`의 `--env` 옵션으로 인스턴스별 `.env` 파일을 명시 지정합니다.
포트 충돌을 피하려면 각 `.env` 파일의 `PORT`를 다르게 설정.

```bash
# 인스턴스별 .env 파일 생성
cp .env.example .env.8003   # PORT=8003 으로 수정
cp .env.example .env.8004   # PORT=8004 으로 수정
...
turkctl --env .env.8004 start
turkctl --env .env.8004 logs
```

> `--env` 생략 시 기본 `.env` 사용 (포트 3000).
