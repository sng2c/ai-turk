		...
		switch (msg.type) {
			...
			case "message_update": {
				...
				if (delta.type === "thinking_start") {
					...
					setThinkingText((prev) => prev + (delta.delta || "")); // 상태 스트립에 스트리밍
			...
			case "response":
				...
				if (msg.command === "get_state" && msg.success && msg.data) {
					...
					// 재접속 복원 — 서버 누적 스냅샷(끊긴 동안 포함 전체)이 로컬보다 완전 → 스냅샷 우선, 없으면 로컬 유지
					if (msg.data.isStreaming && typeof (msg.data as any).thinkingText === "string" && (msg.data as any).thinkingText) {
						setShowThinking(true);
						setThinkingText((prev) => (msg.data as any).thinkingText || prev);
					}
					// 텍스트 스트림 이어받기 — 끊긴 동안의 응답 델타도 서버 누적본으로 복원, 이후 델타가 자연 연결
					if (msg.data.isStreaming && typeof (msg.data as any).streamingText === "string") {
						streamingTextRef.current = (msg.data as any).streamingText;
						setStreamingText((msg.data as any).streamingText);
					}
			...
			case "auto_retry_start": {
				...
				setThinkingText(`⚠️ 재시도 ${(msg as any).attempt}/${(msg as any).maxAttempts} — ${brief} (${Math.round(((msg as any).delayMs ?? 0) / 1000)}초 후)`);
				break;
			}
			case "auto_retry_end":
			...
			case "compaction_start":
				...
				if ((msg as any).reason === "manual") {
					setThinkingText("🧹 컴팩트 진행 중..."); // 상태 스트립 — 메인 출력 불변
