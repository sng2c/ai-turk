...
export default function App() {
		...
		switch (msg.type) {
			...
			case "response":
				...
				if (msg.command === "get_state" && msg.success && msg.data) {
					...
					if (msg.data.model) { const m = msg.data.model; setCurrentModel(m.provider ? `${m.provider}/${m.name || m.id}` : (m.name || m.id || "")); supportedThinkingLevelsRef.current = m.thinkingLevelMap ? THINKING_ORDER.filter(k => (m.thinkingLevelMap as any)[k] != null) : ["off"]; }
	...
	return (
		<div className="turk-app" style={!restored || modelChanging ? { opacity: 0.4, pointerEvents: "none" } : undefined}>
			<header className="turk-header" style={!restored || modelChanging ? { pointerEvents: "none" } : undefined}>
